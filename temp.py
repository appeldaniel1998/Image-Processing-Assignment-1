...
def histogramToImg(imOrig: np.ndarray, imgType: str, sections: np.ndarray, optimalPoints: np.ndarray, hist: np.ndarray) -> np.ndarray:
    ...
    if imgType == "gray":
        ...
        YChannel = yiqImg[..., 0]
        channelShape = YChannel.shape
        YChannel = YChannel.reshape(channelShape[0] * channelShape[1])
        for secInd in range(1, len(sections)):
            for i in range(len(YChannel)):
                if sections[secInd - 1] <= YChannel[i] <= sections[secInd]:
                    YChannel[i] = optimalPoints[secInd - 1]
        YChannel = YChannel.reshape(channelShape[0], channelShape[1])
        yiqImg[..., 0] = YChannel
        yiqImg[..., 0] /= 255
